...
def test_basic_region_selection():
    ...
    selector = ROISelector(min_pixels=5, structure="4-connectivity")
...
def test_custom_connectivity():
    ...
    selector_default = ROISelector(min_pixels=2, structure="4-connectivity")
    with pytest.warns(UserWarning, match="No regions found above min_pixels threshold."):
    ...
    selector_diag = ROISelector(min_pixels=2, structure="8-connectivity")
    ...
    selector_custom = ROISelector(min_pixels=2, structure=custom_structure)
...
def test_empty_mask_returns_empty():
    arr = np.full((4, 4), np.nan)
    mask = make_pixel_mask(arr)
    selector = ROISelector(min_pixels=1, structure="4-connectivity")
    result = selector.select_regions(mask)
    assert np.all(np.isnan(result.mask))
...
def test_zeros_are_regions_nans_are_excluded():
    ...
    mask = make_pixel_mask(arr)
    selector = ROISelector(min_pixels=3, structure="4-connectivity")
...
def test_multiple_large_regions_are_all_included():
    ...
    arr[4:6, 4:6] = True
    mask = make_pixel_mask(arr)
    selector = ROISelector(min_pixels=4, structure="4-connectivity")
    result = selector.select_regions(mask)
...
def test_min_pixels_threshold_variation():
    ...
    selector2 = ROISelector(min_pixels=5, structure="4-connectivity")
...
def test_touching_regions_are_separated():
    ...
    arr[3, 1:3] = True
    mask = make_pixel_mask(arr)
    selector = ROISelector(min_pixels=2, structure="4-connectivity")
    result = selector.select_regions(mask)
    assert np.sum(~np.isnan(result.mask)) == 6
    selector8 = ROISelector(min_pixels=6, structure="8-connectivity")
...
def test_structure_input_variants():
    ...
    selector_none = ROISelector(min_pixels=4, structure=None)
    result_none = selector_none.select_regions(mask)
    assert np.sum(~np.isnan(result_none.mask)) == 4
    # String
    selector_str = ROISelector(min_pixels=4, structure="4-connectivity")
    ...
    selector_arr = ROISelector(min_pixels=4, structure=structure_arr)
